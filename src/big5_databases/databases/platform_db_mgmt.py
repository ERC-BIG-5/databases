...
class PlatformDB(DatabaseManager):
    ...
    def safe_submit_posts(self, posts: list[Union[DBPost, PostModel]]) -> list[PostModel]:
        ...
        try:
            ...
            with self.get_session() as session:
                ...
                submitted_posts = self._submit_posts(filtered_posts)
                return submitted_posts
    ...
    def _get_cached_stats(self) -> Optional[DBDatabaseStats]:
        ...
        with self.get_session() as session:
            if stats := session.query(DBDatabaseStats).one_or_none():
                return stats.model()
        return None
    ...
    def calc_db_content(self, force_refresh: bool = False) -> DatabaseBasestats:
        ...
        """
        cached_stats: Optional[DatabaseStatsModel] = self._get_cached_stats() if not force_refresh else None
