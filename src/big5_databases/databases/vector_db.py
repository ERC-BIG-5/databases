...
class VectorDBManager:
    ...
    def create_table(self, table: LanceTable) -> Table:
        schema = pa.schema([pa.field("vector", pa.list_(table.type_, list_size=table.size))])
    ...
    def get_table(self, table_name: str) -> Table:
        if table_name not in self.tables:
            ...
            self.tables[table_name] = self.db.open_table("my_table_async")
        return self.tables[table_name]

    def add_data(self,table:str, data: dict[str,Any]) -> None:
